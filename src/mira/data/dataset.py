"""RocketLeagueDataset: load time-aligned 4-perspective clips of Rocket League matches.
...
class RocketLeagueDataset:
    ...
    @classmethod
    def from_local(cls, path: str | Path, **kwargs) -> "RocketLeagueDataset":
    ...
    def from_hub(
        ...
        **kwargs,
        ...
        The repo holds each split under its own prefix: `{split}/index.json` + `{split}/dataset_*.tar`
        (e.g. `train/`, `test/`). Pass `split` to choose one; only that prefix is downloaded. `subdir`
        overrides the prefix for a non-split layout. Requires the `hf` extra.
        ...
        `datasets.load_dataset("webdataset", data_files="hf://datasets/<repo>/<split>/*.tar", ...)`.
        """
