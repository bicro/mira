"""The clip-length feasibility guard in ``create_loader`` / ``RocketLeagueDataset.max_clip_frames``.
...
from mira.data.dataset import RocketLeagueDataset
...
def test_max_clip_frames_at_source_rate(tmp_path):
    ...
    ds = RocketLeagueDataset.from_local(_write_index(tmp_path, [[80, 40]]))
...
def test_max_clip_frames_accounts_for_stride(tmp_path):
    ...
    ds = RocketLeagueDataset.from_local(_write_index(tmp_path, [[80]]))
    assert ds.max_clip_frames(target_fps=10) == 40


def test_max_clip_frames_takes_the_longest_across_matches(tmp_path):
    ds = RocketLeagueDataset.from_local(_write_index(tmp_path, [[40], [80], [20, 20]]))
