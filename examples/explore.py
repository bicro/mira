...
def _():
    ...
    from mira.data import RocketScienceDataset, physics, viz

    return RocketScienceDataset, base64, json, mo, physics, viz
...
@app.cell
def _(RocketScienceDataset, local_in, mo, repo_in, split_in):
    try:
        if local_in.value.strip():
            ds = RocketScienceDataset.from_local(local_in.value.strip())
        else:
            ds = RocketScienceDataset.from_hub(repo_in.value.strip(), split=split_in.value.strip() or None)
        _err = None
